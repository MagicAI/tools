    ...
    with open(annfpath, 'a') as fa:
        for ann in anns:
            if args.with_catenm:
                ...
                catenm = args.catenms.index(ann[0])
...
def parse_args():
    ...
    parser.add_argument('--catenms', nargs='+', type=str, default=args.catenms_train)
    ...
    for dirpath, fpaths in walk(args.imgpath, args.regex):
        ...
        for oimgfpath in fpaths:
            ...
            if args.simg:
                ...
                img_show = draw_bboxes(simg.copy(), parse_anns(sanns))
            if args.oimg:
                img_show = draw_bboxes(oimg.copy(), parse_anns(oanns))
