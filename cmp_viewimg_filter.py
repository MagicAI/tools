...
if __name__ == '__main__':
    ...
    for dirpath, fpaths in walk(imgpath0, regex=args.regex):
        ...
        while 1:
            ...
            imgs = []
            show_tag = False
            catenms_train = ['car', 'truck', 'bus', 'pedestrian', 'bicycle', 'cone']
            for i, imgpath in enumerate(args.imgpaths):                
                imgfpath = imgpath / rel_imgfpath0        
                if not imgfpath.exists():
                    break         
                ...
                if args.annpaths is not None:
                    ...
                    annfpath = Path('/'.join(str(annfpath).split('/')[:-1] + ['labels'] + str(annfpath).split('/')[-1:]))
                    print(annfpath)
                    if annfpath.exists():
                    ...
                    # print(anns)       
                    if anns:
                        anns = [ann for ann in anns if ann['cate'] == '5']
                    if len(anns) > 0:
                        show_tag = True
            ...
            if args.dst:
                ...
                cv2.imwrite('{}/{}'.format(dst, rel_imgfpath0.with_suffix('.png').name), img_show)
